...
std::wstring modAlphaChiper::getValidText(const std::wstring& not_valid_open_text){
	std::wstring validText = not_valid_open_text;
		for(int i=0; i< validText.length(); i++){
		if((modAlphaChiper::list_of_alph_for_valid.find(validText[i])) != -1){
			validText[i] = modAlphaChiper::numAlpha[modAlphaChiper::list_of_alph_for_valid.find(validText[i])];
		}
